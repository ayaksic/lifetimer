...
Changing timer pages recreates Apple's remote report with the new filter while retaining one stable report context. The host writes only the selected presentation (`day-flow`, `week-grid`, and so on) to the existing App Group so the sandboxed extension can align its rendering. The local enable preference remains on, authorization is not requested again, and Screen Time durations never leave the extension.
