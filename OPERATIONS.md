...
- Screen Time uses one stable report context for every page. Before a page change, the host writes only the selected presentation to the App Group, then recreates the remote report using an identity derived from the page, query reference date, and lifetime start. This refreshes the filter without changing the persisted toggle, requesting authorization again, or moving activity durations out of the extension.
...
7. Enable the Screen Time overlay, approve access, compare aggregate green screen-on duration with Settings > Screen Time, verify the shorter pages use hourly buckets, confirm page swipes/double taps/long presses still work, and toggle both overlays independently.
