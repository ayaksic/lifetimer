...
| Life Timer Screen Time Report | iOS ExtensionKit | Privacy-preserving current-device activity-duration overlay | Embedded in the iOS app under `Extensions` |
...
| Screen Time screen-on overlay | Apple Device Activity report data authorized by the device owner | Sandboxed Screen Time report extension only | Extension renders aggregate duration; host does not receive activity records; local enable preference is not synchronized |
...
- HealthKit: the iPhone/iPad host requests read-only sleep-analysis access only when the owner enables Sleep from the main timer or Diagnostics. Light blue represents in-bed samples; deep blue represents asleep samples. Overlapping samples from multiple sources are merged by category before totals or drawing.
- Screen Time: the iPhone/iPad host requests individual Family Controls authorization only when the owner enables Screen from the main timer or Diagnostics. The report filter leaves Apple's device selector unset, which means the device currently running Life Timer only; it does not aggregate other devices sharing Screen Time. The Device Activity report extension renders hourly, daily, or weekly aggregate buckets as a soft green hatch whose density represents screen-on duration, plus a compact badge with represented duration and percentage of elapsed time on the current page. The current bucket is clipped at the report time so future timer space remains untouched; exact session timestamps are not exposed. HealthKit sleep and Screen Time are independent, so their overlays may overlap.
- Overlay controls: compact `Sleep` and `Screen` controls at the top-right of the main timer use the same local preferences and authorization flows as Diagnostics. Colored is on; warm-white is off.
- Screen Time uses one stable report context and report view for every page. Before a page change, the host writes only the selected presentation to the App Group, then updates the report filter without forcing the remote extension to be destroyed and recreated. The report remains outside the timer's animation timeline. While Screen is enabled, hour/day pages refresh at five frames per second and week-or-longer pages refresh once per second; the extension renders at most four clipped hatch-density layers per visible timer region rather than per-pixel fill calls.
...
7. Enable the Screen Time overlay, approve access, compare aggregate green screen-on duration with Settings > Screen Time while the current device—not All Devices—is selected, verify the shorter pages use hourly buckets, confirm page swipes/double taps/long presses still work, and leave July Week 4 visible long enough to confirm the report loads without a thermal warning, freeze, watchdog termination, or report-process restart.
...
| Screen Time screen-on overlay | Screen Time remains authoritative inside Apple's report extension; Life Timer stores only a local enable preference and does not export activity records |
